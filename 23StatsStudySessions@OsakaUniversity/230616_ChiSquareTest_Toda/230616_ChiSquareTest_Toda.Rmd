...
以上のように、χ²検定は、データの分布形状に縛られることなく、カテゴリ間の関連性や「データが理論的な分布」に従っているかを確認する非常に便利な手法です。χ²検定を使用するとき、データ自体が特定の確率分布に従う必要はないことを理解することが重要です。しかし、χ²検定の統計量（χ²の値）は、確率分布（具体的にはχ²分布）に従うという前提が存在します。この前提を把握しておくことで、検定結果の解釈とその有効性が適切に理解できます。
