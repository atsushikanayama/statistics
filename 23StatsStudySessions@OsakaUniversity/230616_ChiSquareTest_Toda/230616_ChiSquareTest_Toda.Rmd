...
以上のように、χ²検定は、データの分布形状に縛られることなく、カテゴリ間の関連性やデータが理論的な分布に従っているかを確認する非常に便利な手法です。ただし、χ²検定を使う際は、χ²の値がχ²分布に従うという前提があることを頭に入れておくことが大切です。
...
χ²分布は、複数の独立した標準正規分布（平均0、分散1の正規分布）の二乗和が従う確率分布です。具体的には、k個の独立した標準正規分布の変数を二乗したものを合計すると、この合計は自由度kのχ²分布に従います。

以下に示すグラフでは、ヒストグラムは2つの独立した標準正規分布の二乗和に基づくデータの分布を示しており、滑らかな曲線は自由度2の理論的なχ²分布を表現しています。このグラフから、二乗和がχ²分布に従うことが視覚的に理解できます。
...
p <- ggplot() +
  geom_histogram(data = data.frame(chi_square), aes(x = chi_square, y = ..density..), bins = 50, alpha = 0.5) +
  stat_function(fun = dchisq, args = list(df = 2), aes(color = "理論的なχ²分布"), size = 1) +
  labs(title = "２つの独立した標準正規分布の二乗和",
       x = "値",
       y = "密度",
       color = "凡例") +
  theme_minimal()
...
- **0以上の値のみ**: χ²分布は0以上の値しか取りません。これは、正規分布の二乗和として定義されるため、負の値が出現しないからです。
...
# イェーツの補正とは二項分布が正規分布に近似するために行われる補正のことで、2×2の分割表のχ2検定に用いられます。特にサンプルサイズが小さい場合に有用です。
