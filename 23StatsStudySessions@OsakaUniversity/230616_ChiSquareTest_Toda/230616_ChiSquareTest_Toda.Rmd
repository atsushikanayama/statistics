...
効果量としては、Cohenのwが一般的に用いられます。その計算式は以下のとおりです：

\[ w = \sqrt{\frac{\chi^2}{N}} \]

ここで、\(\chi^2\)はχ²値で、\(N\)は全サンプル数です。Cohen's wの計算は、カイ二乗統計量をサンプルサイズで正規化し（これにより、標本サイズの影響を取り除く）、その結果の平方根をとることで行われます。

Cohen's wは標準化された効果量であり、単に統計的な有意性だけではなく、その結果が実質的にどれだけ意味を持つか、つまりその効果の大きさを評価するために用いられます。この指標は、観測データが期待される分布からどの程度逸脱しているかを示し、その逸脱度がどれだけ大きいかを理解するのに役立ちます。
