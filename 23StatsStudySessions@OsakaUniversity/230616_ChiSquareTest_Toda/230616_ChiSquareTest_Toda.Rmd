...
if(all(expected >= 5)){
  print("期待度数は全て5以上なので、χ²検定を適用可能です。")
} else {
  print("期待度数が5未満のセルが存在するため、χ²検定を適用することはできません。")
}
```
...
```{r, error=TRUE, include=TRUE}
# データの準備
data_matrix <- matrix(c(14, 5, 6, 15), nrow = 2, byrow = TRUE)
rownames(data_matrix) <- c("Female", "Male")
colnames(data_matrix) <- c("Agree", "Disagree")

# データの視覚化
barplot(data_matrix, beside = TRUE, col = c("lightblue", "salmon"), 
