...
このコードでは乱数を用いてデータを生成し、それを用いて線形回帰モデルを作成しました。このモデルの自由度はデータの数からパラメータの数（この場合、2つ）を引いた値です。
...
p <- p + 
  ...
  annotate("text", x = chi_squared, y = 3, 
           label = paste("Chi-squared =", chi_squared), 
           vjust = 0, hjust = 0.2, colour = "black")
...
p <- p + 
  ...
  annotate("text", x = x_squared, y = 3, 
           label = paste("X-squared =", x_squared), 
           vjust = 0, hjust = 0.5, colour = "black")
...
p <- p + 
  ...
  annotate("text", x = critical_value, y = 3, 
           label = paste("Critical Value =", round(critical_value, 2), "(α = 0.05)"), 
           vjust = 0, hjust = 0.7, colour = "black")
...
barplot(x2matrix, beside = TRUE, col = c("lightblue", "salmon"), 
        legend.text = rownames(data_matrix), ylim = c(0, max(data_matrix)*1.2), 
...
p <- p + 
  ...
  annotate("text", x = mcnemars_chi_squared, y = 3, 
           label = paste("McNemar's Chi-squared =", mcnemars_chi_squared), 
           vjust = 4, hjust = 0.5, colour = "black")
...
# データの準備
data <- data.frame(
  Story = c(rep("Forgive", times = 7+2), rep("Do not forgive", times = 23+8)),
  Choice = c(rep("Separate", times = 7), rep("Together", times = 2), rep("Separate", times = 23), rep("Together", times = 8))
)
