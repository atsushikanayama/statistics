...
「子どもたちはストーリーの結果に基づき、「一緒に遊ぶ」か「別々に遊ぶ」かを選んだ。この選択が偶然の分布に従っているかどうかを調べるために、イェーツの補正を用いたマクネマー検定を実施した（McNemar's χ²(1) = 16, p < .001）。この結果、選択が偶然の分布に従っているという帰無仮説は明確に棄却された。つまり、ストーリーの結果が子どもたちの「一緒に遊ぶ」か「別々に遊ぶ」の選択に統計的に有意な影響を与えていることが示唆された。」
